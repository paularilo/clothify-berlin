...
from bsf_package.bsf_logic.plots import plot_rating_berlin, plot_price_berlin,  plot_hist
...
if choice_district == 'Berlin':
 ...
 'Emboidery & Clothing alternation','Fashion accessories','Footwear','Formal wear','General clothing store','Hat shop','Home supplies','Jeans shop', 'Jewelry store', 'Leather store','Maternity store',"Men's clothing",'Optical store','Outlet store','Pet store','Plus size clothing','Second hand clothing','Shopping mall','Sportswear','Swimwear','T-shirt shop','Underwear','Vintage clothing store','Wholesalers',"Women's clothing",'Work clothing','Youth clothing'))
    ...
    choice_shop = st.sidebar.selectbox('Choose a shop type', ('Baby clothing', 'Bag shop','Beauty supplies','Bridal store', "Children's clothing",'Costume store','Department store',
 'Emboidery & Clothing alternation','Fashion accessories','Footwear','Formal wear','General clothing store','Hat shop','Home supplies','Jeans shop', 'Jewelry store', 'Leather store','Maternity store',"Men's clothing",'Optical store','Outlet store','Pet store','Plus size clothing','Second hand clothing','Shopping mall','Sportswear','Swimwear','T-shirt shop','Underwear','Vintage clothing store','Wholesalers',"Women's clothing",'Work clothing','Youth clothing', 'All shops'))
...
if st.session_state.button_on:
    ...
    if choice_district == 'Berlin':
        ...
        df = df[df["neighbourhood_group"] == choice_district]
        amount_shops = len(df) # amount of shops of that type in the selected district and category
        #st.markdown(len(df1))


        if choice_shop != 'All shops': # if specific shop
            if amount_shops > 0:
                with col1:
                ...
                with col2:
                    ...
                    st.markdown(f'The mean rating of {choice_shop.capitalize()} in {choice_district} is of {mean_rat} stars.')
        ...
        elif choice_shop == 'All_shops':
            st.markdown(f'Fill in this')
            col1, col2 = st.columns([6,5])
            with col1:
                st.header(f'There are {amount_shops} establishments in {choice_shop.capitalize()} in {choice_district}') # uppercase the shop type
                dist = search_venue(df)
                heat = heatmap_venues(dist)
                st_folium(heat,width=500, height=500)
            with col2:
